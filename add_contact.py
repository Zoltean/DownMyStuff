...
class AddContactDialog(QDialog):
    ...
    def validate_and_correct_data(self):
        ...
        if not sename:
            QMessageBox.warning(self, "Помилка", "Прізвище є обов'язковим полем.")
            return None, None, None, None, None, None, None
        if not name:
            QMessageBox.warning(self, "Помилка", "Ім'я є обов'язковим полем.")
            return None, None, None, None, None, None, None
        if not phone_number:
            QMessageBox.warning(self, "Помилка", "Номер телефону є обов'язковим полем.")
            return None, None, None, None, None, None, None
        ...
        if sename is not None:
            try:
                with sqlite3.connect('database.db') as connection:
                    ...
                    cursor.execute("""
                        INSERT INTO contacts (sename, name, f_name, phone_number, address, address_NP, email)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (sename, name, f_name, phone_number, address, address_NP, email))
                    connection.commit()
                QMessageBox.information(self, "Успіх", "Контакт успішно додано.")
                self.accept()
            except sqlite3.DatabaseError as e:
                QMessageBox.critical(self, "Помилка", f"Помилка при збереженні контакту: {e}")
